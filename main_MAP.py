...
class Model(nn.Module):
    def __init__(self, met_locs, microbe_locs, temp_scheduled = 1, num_local_clusters = 6, K = 2, beta_var = 16.,
                ...
                l1= 1, marginalize_z = True, adj_mvar = False):
        ...
        self.adj_mvar = adj_mvar
        if self.adj_mvar:
            self.meas_var_mult = torch.tensor(meas_var).float()
        else:
            self.meas_var = torch.tensor(meas_var).float()
    ...
    def initialize(self, seed):
        ...
        self.initializations['mu_bug'] = MultivariateNormal(torch.zeros(self.embedding_dim),
                                                            (self.mu_var_bug/5)*torch.eye(self.embedding_dim))
    ...
    def forward(self, x, y):
        ...
        if self.adj_mvar:
            self.meas_var = self.meas_var_mult*np.sqrt(self.K*self.L)*torch.var(out_clusters.detach().flatten())
...
if __name__ == "__main__":
    ...
    parser.add_argument("-learn", "--learn", help="params to learn", type=str, nargs='+', default = ['mu_bug'])
    parser.add_argument("-lr", "--lr", help="params to learn", type=float, default = 0.1)
    ...
    parser.add_argument("-case", "--case", help="case", type=str, default = 'debug')
    parser.add_argument("-N_met", "--N_met", help="N_met", type=int, default = 6)
    parser.add_argument("-N_bug", "--N_bug", help="N_bug", type=int, default = 6)
    ...
    parser.add_argument("-prior_meas_var", "--prior_meas_var", help = "prior measurment variance", type = float, default = 1)
    parser.add_argument("-iterations", "--iterations", help="number of iterations", type=int,default = 22001)
...
info = 'lr' + str(args.lr) + '-linear'*(args.linear) + '-marg_z'*args.mz + '-adj_lr'*args.adjust_lr + \
...
true_vals = {'y':y, 'beta':gen_beta, 'alpha':gen_alpha, 'mu_bug': mu_bug,
             ...
             'r_bug': np.log(1.2*r_bug), 'r_met': np.log(1.2*r_met), 'z': gen_z, 'w': gen_w, 'pi_met':np.expand_dims(np.sum(gen_z,0)/np.sum(np.sum(gen_z)),0),
             'pi_bug':np.expand_dims(np.sum(gen_w,0)/np.sum(np.sum(gen_w)),0), 'bug_locs': gen_bug_locs, 'met_locs':gen_met_locs,
             'e_met': np.expand_dims(np.sum(gen_z,0)/np.sum(np.sum(gen_z)),0),'b': mu_met}
if args.linear:
...
for name, parameter in net.named_parameters():
    ...
    if name == 'r_bug' or name == 'r_met' or name == 'e_met':
...
if args.mz:
    param_dict[args.seed]['z'] = [net.z_act.numpy()]
if 'w' not in params2learn:
...
for epoch in np.arange(start, args.iterations):
    ...
    if epoch%10 == 0:
        ...
        mvar.append(net.meas_var)
        loss_dict_vec.append(net.MAPloss.loss_dict)
        if epoch%100==0 and epoch > 0 and 'mu_bug' in params2learn or 'all' in params2learn:
    ...
    if epoch%10 == 0:
        ...
        if args.mz:
            param_dict[args.seed]['z'].append(net.z_act.numpy())
        if net.linear:
    ...
    if (epoch%1000 == 0 and epoch != 0):
        ...
        fig3, ax3 = plot_loss(fig3, ax3, args.seed, np.arange(start, epoch + 1, 10), loss_vec, lowest_loss=None)
        fig3.tight_layout()
        ...
        if 'beta' not in params2learn:
            ...
            mapping['bug'] = pd.Series(mapping['bug']).sort_index()
        plot_xvy(path, net, x, train_out_vec, last_mod, y, gen_z, gen_w, mapping, args.seed)
...
#     parser.add_argument("-iterations", "--iterations", help="number of iterations", type=int,default = 21)
...
#     parser.add_argument("-l1", "--l1", type=int, default=0)
#     parser.add_argument("-mz","--mz", type = int, default = 0)
#     args = parser.parse_args()
