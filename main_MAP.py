...
class Model(nn.Module):
    ...
    def forward(self, x, y):
        ...
        self.meas_var = np.var(out_clusters.detach().numpy().flatten()/10)
...
info = 'lr' + str(args.lr) + '-linear'*(args.linear) + \
       '-l1'*(args.l1) + '-'*(1-args.linear) +args.nltype*(1-args.linear) + \
