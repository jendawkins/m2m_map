...
class Model(nn.Module):
    def __init__(self, met_locs, microbe_locs, temp_grouper = 10, temp_selector = 10, L = 2, K = 2, beta_var = 10, c_var = 1,
                 mu_var_met = 10, mu_var_bug = 10, r_scale_met = 10, r_scale_bug = 10):
    ...
    def forward(self, x):
        ...
        w = torch.softmax(-self.temp_grouper*(euc/self.r_bug.unsqueeze(1).repeat(1,euc.shape[1])), dim = 1)
        ...
        z = torch.softmax(-self.temp_grouper*(euc/self.r_met.unsqueeze(1).repeat(1,euc.shape[1])),dim = 1)
...
if __name__ == "__main__":
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(x)):
        ...
        for met in range(test_targets.shape[1]):
            ...
            ax_dict2[fold][met].hist(preds[:,met], range = (total.min(), total.max()), label = 'guess', alpha = 0.5)
        ...
        for cluster in range(gen_z.shape[1]):
            ...
            ax_dict3[fold][cluster].hist(preds[:,met_ids].flatten(), range = (total.min(), total.max()), label = 'guess', alpha = 0.5)
